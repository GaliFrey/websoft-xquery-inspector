...
Она проверена в сборках 434, 906, 1132, 1333 и 1525 с MSSQL и PostgreSQL;
для обоих провайдеров во всех этих сборках подтверждён полный протокол.
На стенде со сборкой 434 визуально подтверждено отображение
эффективного XQuery, диагностических данных, ошибки несовместимой версии
контракта и ограничение размера.

13.09.2026 обязательный набор проверок серверного агента с DLL `1.3.0`
успешно прошёл в сборках 434, 906, 1132 и 1333 с MSSQL и PostgreSQL.
Это подтверждает текущий автоматизированный набор проверок, но не заменяет
ручные пункты полного протокола. 14.09.2026 автоматизированный набор и ручные
пункты полного
протокола успешно прошли в сборке 1525 с MSSQL и PostgreSQL, UniBridge
`2.26.8.12`, DLL `1.3.0` и Microsoft.NETCore.App `9.0.19`. На PostgreSQL
`16.3` подтверждены параметры `varchar` и `varchar[]`.
...
автоматически по матрице и эталонным манифестам.
...
- Node.js `22.23.1`;
- .NET SDK из feature band `10.0.1xx`: `global.json` выбирает последнюю
  установленную версию этой линейки; проект проверен с SDK `10.0.111`;
- targeting packs `net6.0` и `net9.0`;
- Git для финальной проверки формата изменений.

Для полной проверки дополнительно нужны среды выполнения .NET `6.0.36` и
`9.0.20`.
...
Release-сборку решения, smoke-тесты в средах выполнения .NET 6 и 9,
контрактные тесты, проверку матрицы совместимости, публичного состава
репозитория, UTF-8 BOM, CRLF и
`git diff --check`. При первой ошибке команда останавливается с
ненулевым кодом. Если нужной среды выполнения нет, такой запуск не
...
| `websoft/websoft-xquery-inspector.html` | Готовое содержимое браузерного интерфейса и серверного обработчика | Скопировать целиком в настраиваемый шаблон с кодом `websoft-xquery-inspector`; это не файл для каталога xHttp |
...
| `websoft/xquery-inspector-compatibility-agent-2.js` | Дополнительный агент ручной части полного протокола | Запускать только на изолированном стенде: агент выполняет два тестовых `tools.xquery()` и пишет полный диагностический JSON в отдельный журнал |
...
экземпляра WebSoft HCM. Затем на каждой комбинации сборки WebSoft HCM и SQL-провайдера
...
Эти smoke-тесты проверяют загрузку и выполнение автономной DLL в обычной
среде .NET.
Они не эмулируют загрузчик `Datex.Core` и не подтверждают совместимость
...
Он запускается внутри WebSoft HCM, использует локальный провайдер UniBridge и
...
контракт, метаданные среды выполнения, известная цепочка рефлексии, SQL и
Count SQL, параметры, длительности, этап ошибки и освобождение ресурсов.

Сценарий массива PostgreSQL, успешная трансляция запроса ровно из 200000
символов Unicode и отклонение синтаксической ошибки самим парсером
UniBridge пока остаются в ручном полном протоколе: некорректный XQuery может
зависнуть в старом провайдере PostgreSQL. Агент проверяет скалярный параметр PostgreSQL
общим параметрическим сценарием, безопасную ошибку подготовки внутри DLL и автоматическое
отклонение 200001 символа до обращения к провайдеру.

Дополнительный ручной агент находится в
[`websoft/xquery-inspector-compatibility-agent-2.js`](websoft/xquery-inspector-compatibility-agent-2.js).
Он проверяет успешную границу в 200000 символов и ошибку парсера на этапе
`invoke-xquery`, а также запускает `IsHierChild()` и `IsHierChildOrSelf()` через
штатный `tools.xquery()` для сравнения с результатом Inspector. Используйте его
только на изолированном стенде: запросы `tools.xquery()` выполняются, а журнал
`xquery_inspector_manual` содержит полный XQuery, SQL и параметры. Агент
поддерживается вручную и не пересобирается `tools/build-compatibility-agent.js`.
...
Агент подтверждает фактическую загрузку DLL, рефлексию, трансляцию и освобождение
ресурсов на конкретном сервере, но по-прежнему не перечисляет коллекцию и не выполняет
сформированный SQL. Неизвестный путь доступа к объектам среды выполнения сначала
добавляется в DLL и локальный имитационный smoke-сценарий,
после чего прогон агента повторяется.

Имя сборки и пространство имён `XQueryInspector` намеренно не содержат префикс
`Websoft.` или `Datex.`. Загрузчик WebSoft HCM 434 считает такие сборки
системными и требует фирменный токен открытого ключа (public key token).
...
Это сгенерированный файл, отслеживаемый Git: вручную изменяются только пять
частей в
...
3. Сохраните шаблон в UTF-8 с BOM. В репозитории для него зафиксированы
...
GET возвращает HTML-фрагмент интерфейса. POST-запрос через AJAX отправляет поля
...
проверку доступа к шаблону. Длина XQuery ограничена 200000 символов Unicode:
интерфейс проверяет строку до отправки, серверный обработчик возвращает
HTTP 413 до загрузки DLL, а
...
- свёрнутую самодиагностику с версиями, цепочкой рефлексии, этапом ошибки и
...
Шаблон не создаёт собственные элементы `html`, `head`, `body` и верхнюю
шапку. Все стили
...
Обновлённый шаблон продолжает показывать устаревший ответ без версии с
...
имя типа значения параметра в среде выполнения.
...
указать наличие этой возможности. Прежний двухаргументный вызов сохранён,
предполагает `UNI_CAP_BASIC` и используется шаблоном для совместимости со старой DLL при
...
- до 16 уровней вложенности `IEnumerable`;
- до 100000 нормализованных узлов суммарно;
- до 1000000 символов Unicode в нормализованных строковых значениях суммарно;
- до 1048576 байт в одном `byte[]`.

Ссылки на `IEnumerable` отслеживаются на активной ветви, поэтому циклическая
...
UniBridge и итераторы, реализующие `IDisposable`, при этом освобождаются штатно.
Они сдерживают рост объёма занимаемой памяти и размера ответа, но не заменяют
лимит времени xHttp.
...
- Нужны активное внешнее SQL-хранилище и
  `tools.spxml_unibridge.Object.provider`.
- Механизм рефлексии опирается на внутренние члены UniBridge и требует
  проверки на каждой поддерживаемой сборке.
...
- `Datex.Core.CoreAssemblyLoadContext` сборки 434 проверяет токен открытого
  ключа (public key token) у сборок с префиксами `Datex.` и `Websoft.`;
